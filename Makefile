...
test-e2e: ## If MANAGEMENT_CLUSTER_ENVIRONMENT is 'eks', run remote e2e tests, otherwise run local e2e tests
	# The idea is that we should remove the requirement of running image building before e2e
	# It should be embedded as part of the 'test-e2e' command
	# This will allow us to remove the publish workflow and the call to 'make e2e-image-build-and-push' from there
	# if 'eks', e2e-image-build-and-push
	# any case that's not 'eks' means a local management cluster -> e2e-image
...
test-e2e-remote: $(GINKGO) $(HELM) $(CLUSTERCTL) kubectl e2e-image-build-and-push build-local-rancher-charts ## Run the end-to-end tests after pushing the e2e image to a remote registry
	$(E2E_RUN_COMMAND)
